...
const DEFAULT_USE_DOCKERISED_SOLC = true;
...
export class Compiler {
    ...
    private async _compileContractsAsync(contractNames: string[], shouldPersist: boolean): Promise<StandardOutput[]> {
        ...
        // console.log(_.uniq(_.sortBy(spyResolver.resolvedContractSources.map(a => `${a.absolutePath}`))));
        ...
        for (const solcVersion of solcVersions) {
            ...
            if (this._useDockerisedSolc) {
                ...
                compilerOutput = compileDocker(this._resolver, this._contractsDir, solcVersion, input.standardInput);
            } else {
